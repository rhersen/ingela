...
function handleTrains() {
  if (this.status >= 200 && this.status < 400) {
    ...
    document.getElementById("trains").outerHTML = html.trains(
...
function handleTrain() {
  if (this.status >= 200 && this.status < 400) {
    ...
    document.getElementById("trains").outerHTML = formatLatestAnnouncement(
      result.TrainAnnouncement,
      stations
    )
  } else {
...
function handleStations() {
  if (this.status >= 200 && this.status < 400) {
    const trainStations = JSON.parse(this.response).RESPONSE.RESULT[0]
      .TrainStation
    stations = {}
    trainStations.forEach(
      entry =>
        (stations[entry.LocationSignature] = entry.AdvertisedShortLocationName)
    )
    getCurrent()
  } else {
    document.getElementById("update").textContent = this.status
